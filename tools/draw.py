...
def get_round_method_dict():
    """Get dictionary round method number to corresponding round method
    string
    """
    desc = caffe_pb2.QuantizationParameter.RoundMethod.DESCRIPTOR
    d = {}
    for k, v in desc.values_by_name.items():
        d[v.number] = k
    return d

def get_round_strategy_dict():
    """Get dictionary round strategy mapping number to strategy name
    """
    desc = caffe_pb2.QuantizationParameter.RoundStrategy.DESCRIPTOR
    d = {}
    for k, v in desc.values_by_name.items():
        d[v.number] = k
    return d
...
def get_layer_label(layer, rankdir):
    ...
    elif layer.type == 'Quantization':
        round_method_dict = get_round_method_dict()
        round_strategy_dict = get_round_strategy_dict()
        node_label = '"%s%s(%s %s)%sBW: %d%sR:[%.2f, %.2f]"' % \
                (layer.name, \
                separator, \
                round_strategy_dict[layer.quantization_param.round_strategy], \
                round_method_dict[layer.quantization_param.round_method], \
                separator, \
                layer.quantization_param.bit_width,\
                separator, \
                layer.quantization_param.range[0],\
                layer.quantization_param.range[1]
                )
    else:
